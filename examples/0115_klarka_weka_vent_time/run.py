...
def determine_ventilation_time_class(events):
    for i in range(0, len(events)):
        ventilation_time = events[i]['times']['ventilation_time']
        if ventilation_time > 900 and ventilation_time <= 1200:
            events[i]['graph_vent_type_1'] = "20"
        elif ventilation_time > 1200 and ventilation_time <= 1500:
            events[i]['graph_vent_type_1'] = "25"
        else:
            events[i]['graph_vent_type_1'] = "unclassified"
...
def prepare(events):
    ...
    for event in events:
        ...
        values = [
            ...
            ('rh_in_diff', round(dp.find_module(event, 'humidity_in')['rh_in_diff'], precision)),

            (class_name, event['graph_vent_type_1']),
        ]
    ...
    for item, _ in values:
        if item == class_name:
            file.write('@attribute class {20, 25}\n')
            continue
...
if __name__ == '__main__':
    ...
    filtered = dp.calculate_ventilation_time(filtered)
    filtered = determine_ventilation_time_class(filtered)
    filtered = storage.filter_downloaded_data_general_attribute(filtered, "graph_vent_type_1", "unclassified")
    ...
    filtered = dp.calculate_relative_humidity_difference(filtered)
    filtered = storage.filter_downloaded_data_one_module_2(filtered, 'humidity_in', 'rh_in_diff', 0.0)
    ...
    for i in range(0, len(filtered)):
        ...
        stat = [
            ...
            ('', ''),
            ('rh_in_diff', round(dp.find_module(filtered[i], 'humidity_in')['rh_in_diff'], precision)),
        ]
