...
def reduce_classes(events):
    for i in range(0, len(events)):
        type = events[i]['graph_type_hum']

        if type == 'big':
            events[i]['g_type'] = 'big'
        elif type == 'small':
            events[i]['g_type'] = 'small'
        else:
...
def prepare(events):
    ...
    for event in events:
        norm_values_temp_in = dp.find_module_measured(event, 'beeeon_temperature_in')
        norm_values_temp_out = dp.find_module_measured(event, 'beeeon_temperature_out')
        norm_values_hum_in = dp.find_module_measured(event, 'beeeon_humidity_in')
        norm_values_hum_out = dp.find_module_measured(event, 'beeeon_humidity_out')
    ...
    for item, _ in values:
        if item == class_name:
            file.write('@attribute class {small, big}\n')
            continue
...
if __name__ == '__main__':
    ...
    filtered = dp.estimate_relative_humidity(filtered, 'beeeon_humidity_in', 'beeeon_humidity_out',
                                             'beeeon_temperature_in')
