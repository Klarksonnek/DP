...
def humidity_info_csv(events, start_shift, end_shift, precision=2):
    ...
    for event in events:
        ...
        out.append(OrderedDict([
            ('start_datetime', event['e_start']['timestamp']),
            #('end_datetime', event['e_end']['timestamp']),
            ...
            #('ventilation_length_1', event['ventilation_length_1']),
            ...
            ('ventilation_length_2', event['ventilation_length_2'])
        ]))
...
def add_ventilation_length_attr(events: list):
    out = []

    res = FilterUtil.min_max_time_interval(events, 240, 420)
    for event in res:
        event['ventilation_length_1'] = 'pet'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 480, 720)
    for event in res:
        event['ventilation_length_1'] = 'deset'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 780, 1020)
    for event in res:
        event['ventilation_length_1'] = 'patnact'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 1080, 1320)
    for event in res:
        event['ventilation_length_1'] = 'dvacet'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 1380, 1620)
    for event in res:
        event['ventilation_length_1'] = 'dvacetpet'
        out.append(event)

    return out


def add_ventilation_length_attr_2(events: list):
    out = []

    res = FilterUtil.min_max_time_interval(events, 240, 420)
    for event in res:
        event['ventilation_length_2'] = 'pet'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 480, 720)
    for event in res:
        event['ventilation_length_2'] = 'deset'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 1080, 1320)
    for event in res:
        event['ventilation_length_2'] = 'dvacet'
        out.append(event)

    res = FilterUtil.min_max_time_interval(events, 1380, 1620)
    for event in res:
        event['ventilation_length_2'] = 'dvacetpet'
        out.append(event)
...
def main(events_file: str, start_shift: int, end_shift: int, output_filename: str,
    ...
    sensor1_events = add_ventilation_length_attr(sensor1_events)
    ...
    sensor2_events = add_ventilation_length_attr_2(sensor2_events)
